...
shash_table_t *shash_table_create(unsigned long int size)
{
	...
	shtt = calloc(1, sizeof(shash_table_t));
	if (!shtt)
		return (NULL);

	shtt->size = size;
	shtt->array = calloc((size_t)shtt->size, sizeof(shash_node_t *));
	if (shtt->array == NULL)
	{
		free(shtt);
		return (NULL);
...
int shash_table_set(shash_table_t *ht, const char *key, const char *value)
{
	...
	n = add_node_s(&ht->array[idx], key, value);
	if (!n)
		return (0);
